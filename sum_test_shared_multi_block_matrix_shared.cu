...
__global__ void sumador_2(int* arreglo, int* result, float N){
	...
	for(int i=1; pow((float)2,(float)i-1) < 160; i++)
	{
		int acceso = pow((float)2,(float)i);
		int offset = pow((float)2, (float)i-1);
		if(threadIdx.x < (10.0/acceso) && (threadIdx.x * acceso + offset) < (N - blockIdx.x * blockDim.x))
		{
				compartida[threadIdx.x * acceso] = compartida[threadIdx.x * acceso] + compartida[threadIdx.x * acceso + offset * 16];
...
// 		int acceso = pow((float)2,(float)i);
// 		int offset = pow((float)2, (float)i-1) ;
// 		if( (( (threadIdx.x / 16) & (offset*2)-1 ) == 0) && ( (threadIdx.x+offset*16) < N*16 - blockIdx.x * blockDim.x))
// 		{
...
int main(int argc, char** argv){
	...
	// dim3 miGrid1D_2(1,1);
	// sumador_2<<<miGrid1D_2, miBloque1D_1>>>(d_arreglo_suma2, d_arreglo_suma1, 2);
